...
void run(struct dso_entry *entry) {
    ...
    if(r != 0) {
	fprintf(stderr, ERRORTEXT("Fatal: Failed to initialize new thread attributes") ": %s\n", strerror(r));
	arcp_release(entry);
	exit(EXIT_FAILURE);
    }
    r = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    if(r != 0) {
	fprintf(stderr, ERRORTEXT("Fatal: Failed to set detached thread attribute") ": %s\n", strerror(r));
	arcp_release(entry);
	exit(EXIT_FAILURE);
    }
    ...
    if(r != 0) {
	fprintf(stderr, ERRORTEXT("Fatal: Failed to create new thread") ": %s\n", strerror(r));
	arcp_release(entry);
	exit(EXIT_FAILURE);
    }
    ...
    if(r != 0) {
	fprintf(stderr, ERRORTEXT("Failed to clean up new thread attributes") ": %s\n", strerror(r));
    }
}
